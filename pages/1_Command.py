...
from core.data   import get_ohlcv_batch, get_ohlcv
import config as cfg

# Lazy imports — only needed when scanner runs
def _lazy_imports():
    from core.vsa      import get_vsa_verdict
    from core.rrg      import get_rrg_data
    from core.signals  import get_trigger_signal
    from core.portfolio import get_portfolio, get_watchlist, add_to_watchlist
    return get_vsa_verdict, get_rrg_data, get_trigger_signal, get_portfolio, get_watchlist, add_to_watchlist

# ==============================================================================
# PAGE CONFIG (only set in app.py — this page is loaded as a sub-page)
# ==============================================================================
...
if regime:
    ...
    # Four pillar cards
    vix_state = "good" if regime.vix_safe else "bad"
    ...
    # Inertia status strip
    if regime.fail_count > 0:
        st.warning(
            f"⚠️ Inertia Lock: {regime.fail_count}/3 days of internal breakdown detected. "
...
# SCAN CONTROLS
# ==============================================================================
c_title, c_opt, c_btn = st.columns([5, 1.2, 1.2])
with c_title:
...
with c_btn:
    if st.button("🔥 RUN SCAN", use_container_width=True, type="primary"):
        ...
        get_vsa_verdict, get_rrg_data, get_trigger_signal, get_portfolio, get_watchlist, add_to_watchlist = _lazy_imports()

        with st.spinner(f"Deploying {mode} scanner..."):
            results = _run_scan(mode, regime, get_vsa_verdict, get_rrg_data,
...
if st.session_state.scan_results:
    ...
    # -- 1. Critical Threats --
    st.markdown("#### 🔴 Critical Actions (Exit / Tighten)")
    if not res["sells"].empty:
        ...
        st.success("✅ No immediate threats detected in portfolio.")
    ...
    if not res["holdings"].empty:
        st.dataframe(
            ...
            column_config={
                ...
                "Alpha":  st.column_config.NumberColumn("Alpha", format="%.2f"),
                "Quad":   st.column_config.TextColumn("RRG State", width=100),
                "Vel":    st.column_config.NumberColumn("Velocity", format="%.2f"),
                "Vol":    st.column_config.NumberColumn("Rel Vol",  format="%.1fx"),
            },
        )
    else:
        st.info("No active holdings to check.")
...
# ==============================================================================
# LEVEL 1 — THE TRAP (Persistent Watchlist)
# ==============================================================================
st.markdown("## 🎯 The Trap")

try:
    _, _, _, _, get_watchlist, add_to_watchlist = _lazy_imports()
    ...
    evt_trap = st.dataframe(
        ...
        column_config={
            "Ticker":        st.column_config.TextColumn("Ticker",    width=70),
            "Sector State":  st.column_config.TextColumn("Sector",    width=120),
            "Signal":        st.column_config.TextColumn("Signal",    width=200),
            "VSA":           st.column_config.TextColumn("VSA",       width=180),
        },
    )

    if evt_trap.selection.rows:
    ...
    with c_add:
        new_ticker = st.text_input("Add ticker to Trap", label_visibility="collapsed",
                                   placeholder="Enter ticker...").upper()
    with c_btn2:
        if st.button("➕ Add to Trap", use_container_width=True) and new_ticker:
...
# ==============================================================================
# SCANNER ENGINE (internal, called only when button pressed)
# ==============================================================================
def _run_scan(mode, regime, get_vsa_verdict, get_rrg_data,
              get_trigger_signal, get_portfolio, get_watchlist, add_to_watchlist=None):
    """
    Runs the dual-mode scanner.
    ...
    # Batch OHLCV
    if mode == "MARKET":
        prog = st.progress(0, text="Scouting universe...")
    else:
        prog = None
    ...
    opportunities, threats, holdings_report = [], [], []
    weak_holdings = []
    ...
    # Swap logic
    has_cash   = True   # simplified — portfolio module handles actual cash
    hot_buys, swaps = [], []
